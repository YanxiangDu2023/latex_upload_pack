...
Stakeholder feedback pointed in the same direction. The separated stage views made the process easier to explain and easier to check than the previous SAP-based way of inspecting the same scope. The evaluation therefore supports the claim that the prototype improved transparency, usability, and traceability for the implemented stages, while still leaving open how far the same improvement would carry to a broader production setting.

More broadly, the Volvo CE case suggests that inspection-oriented representations can add value in ERP-based enterprise data processing when business users need to see how inputs, maintained rules, and staged transformations connect to the reported result.
...
The main practical contribution of this thesis is the development of a working prototype that lets users inspect selected TMC stages in a way that was not previously available in the Volvo CE setting. Rather than replacing SAP, the prototype complements the existing implementation with an environment for upload review, staged inspection, and discussion of intermediate results. Subject to further development, full testing, and internal infrastructure integration, the prototype could potentially support internal use in a future TMC cycle.

Its originality lies in how the selected stages were re-expressed for inspection. Preparation, market view, adjustment, machine line split, and resplit are shown as explicit reviewable stages, and users can work with filterable tables, summaries, charts, and CSV export instead of relying only on final aggregated outputs or technical SAP structures. The contribution is therefore not a new algorithm. It is a concrete design contribution that shows one way to make an enterprise data-processing setting more inspectable when it depends on heterogeneous inputs, maintained rules, and sequential transformations.

\subsection{Academic Contributions}
On the academic side, the thesis contributes a concrete example of how transparency, traceability, usability, and inspectability problems in ERP-based enterprise data processing can be addressed through an implemented artefact rather than only discussed conceptually. The Volvo CE case is specific, but the underlying issue is wider. Prior work on enterprise data pipelines and ETL shows that many such workflows depend on heterogeneous inputs and staged transformations \cite{vassiliadis2009etl}. Research on heterogeneous data integration points in the same direction by showing that such workflows often depend on combining different input structures before later processing can take place \cite{putrama2024heterogeneous}. In SAP-related settings, embedded rules and system-specific structures can then make these workflows difficult for business users to inspect \cite{weber2022sap}. Similar usability concerns have also been reported in ERP user environments \cite{calisir2004erp}.
...
Future work should address both system scope and research scope. On the system side, the artefact could first be extended to later workflow stages such as full Total Market Calculation, Restatement, and Reporting, so that a larger part of the workflow can be inspected in one environment. This would make it easier to follow how early preparation and rule-based redistribution affect later market-share results rather than stopping at intermediate stages only.

A second direction concerns configurability and deployment readiness. More mappings, workflow conditions, and calculation rules could be moved out of backend code and into configurable structures, which would make the system easier to maintain as rules change over time. In addition, broader internal use would require stronger operational capabilities such as enterprise-grade authentication, role-based access control, workflow orchestration, monitoring, backup and recovery, and deployment within Volvo CE's internal infrastructure.

Further work is also needed on traceability and analytical support. The current prototype exposes intermediate stages, but it does not yet provide full end-to-end row-level lineage. A useful continuation would therefore be to strengthen drill-down from outputs to original inputs, make rule applications more explicit, preserve clearer version history for source and matrix changes, and support comparison between workflow runs or yearly cycles. This would improve the artefact not only as a review tool, but also as a support tool for investigation, explanation, and change analysis.

A fourth direction concerns evaluation over time. The artefact should be studied over a longer period in regular organizational use and with additional yearly datasets. This would make it possible to assess its effects on validation work, communication between business and IT, user confidence in workflow outputs, and the practical value of stage-level inspection in recurring workflow cycles.

Finally, future research should examine transferability beyond the Volvo CE case. One useful step would be to apply the same inspection-oriented design approach in another organizational setting or another enterprise data-processing setting with similar characteristics, such as heterogeneous inputs, maintained rules, and staged transformations. This would help distinguish more clearly between findings that are specific to the Volvo CE TMC setting and design principles that are transferable to ERP-based enterprise data processing more generally.
