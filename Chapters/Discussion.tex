...
A key contribution is that the prototype exposes intermediate workflow outputs, including preparation, adjustment, and split-related results, instead of only presenting final aggregated figures. Through filterable tables, summary values, charts, and CSV export, users can inspect how values are formed, compare outputs with source data, and validate whether the implemented logic behaves as expected. More broadly, this contributes practical design knowledge on how complex enterprise data-processing workflows can be made more inspectable through staged output exposure and workflow-oriented review views.

For the Strategy and Business Intelligence function at Volvo CE, the prototype offers a new digital way of working with TMC-related data. At the same time, the case illustrates a more general point: where enterprise workflows rely on heterogeneous inputs, embedded rules, and sequential transformations, a separate inspection-oriented prototype can improve reviewability without replacing the underlying operational platform.

\subsection{Originality}

The originality of this thesis lies in the independent design and development of a dedicated web-based prototype that re-expresses selected SAP-embedded workflow stages as inspectable processing layers. Before this work, there was no such solution in the studied Volvo CE context.
