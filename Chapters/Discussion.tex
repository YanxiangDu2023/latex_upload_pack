...
The prototype answers Sub-RQ1 through a web-based input-management layer that treats workflow files as maintained artefacts rather than as temporary uploads. The design provides dedicated interfaces for both source datasets and matrix inputs, and each uploaded file is validated, normalized, and stored together with metadata such as file type, timestamp, processing status, row count, and version information. This creates a controlled input architecture in which users can identify which files are currently available, which version is active, and whether a dataset is ready for downstream processing.

The answer is not only technical but also practical from a workflow perspective. The frontend supports viewing uploaded data, downloading stored inputs, and saving edited content as a new version. As a result, input handling becomes a traceable and reusable process rather than a one-time file-transfer step. Users can inspect rows directly in the interface, confirm that required matrix and source files are present, and review whether the current inputs match the intended workflow state before running later stages.

The 2024 evaluation showed that different source and matrix files could be ingested, stored, retrieved, downloaded, and re-saved correctly. Within the scope of the Volvo CE case, this demonstrates that heterogeneous workflow inputs can be managed through a structured web-based architecture that improves accessibility, control, and inspectability compared with ad hoc file handling, while also illustrating a more general approach to input management in ERP-based enterprise workflows.





% \subsection{Answer to Sub-RQ2: How can the preparation, market-view derivation, adjustment, machine line split, and resplit stages of the TMC workflow be re-expressed in a web-based environment so that intermediate transformations and derived results become inspectable and traceable for business users?}


% Sub-RQ2 is addressed by re-expressing the preparation, market-view derivation, adjustment, machine line split, and resplit stages as explicit web-based workflow layers with persisted intermediate outputs, rule-visible fields, and stage-specific inspection views.
...
The prototype answers Sub-RQ2 by re-expressing selected workflow activities as explicit web-based layers with persisted intermediate outputs and stage-specific inspection views. In the Volvo CE case setting, this covers preparation, market-view derivation, adjustment, machine line split, and resplit. Instead of treating these activities as embedded operations inside SAP execution logic, the prototype separates them into named stages that can be viewed, triggered, and inspected individually.

This staged representation makes different forms of workflow logic visible at the point where they matter. Preparation validates, cleans, and aligns CRP, SAL, and OTH data with matrix-based rules while exposing mapped dimensions and control fields. Market-view derivation exposes how source data is reorganized into the structures needed for later processing. Adjustment remains visible as a separate step in which source-detail and derived rows can be reviewed together. Machine line split and resplit make redistribution logic inspectable by exposing split ratios, before/after totals, and redistributed rows rather than only presenting aggregated end results.

The inspection mechanism is equally important to the answer. Pipeline Viewer provides workflow-level visibility into stage order, readiness, and dependencies, while Layer Detail pages provide row-level outputs, summaries, and rule-related fields for closer validation. Together, these design choices turn selected SAP-embedded transformations in the Volvo CE case into explicit and traceable workflow stages, while also illustrating how complex ERP-based enterprise workflows can be re-expressed in a more business-accessible form.
...
The evaluation indicates that the prototype improves transparency, usability, and traceability within the implemented scope, compared with the existing SAP-based workflow representation.

In the SAP-based representation, the TMC workflow is organized through technical calculation steps and system objects such as matrix structures, planning functions, planning sequences, and reporting objects. Although the process can be executed in SAP, business-side inspection of intermediate logic is less direct, especially when users need to trace how outputs are formed from CRP, SAL, OTH, and matrix-based rules.

The prototype improves this representation by exposing the workflow through dedicated web-based inspection flows. Input handling is centralized through upload and matrix-management pages, workflow progress is shown through Pipeline Viewer, and intermediate results are made available through Layer Detail pages with row-level outputs, summaries, and rule-related fields. This makes preparation, market-view derivation, adjustment, machine line split, and resplit-related transformations easier to inspect and validate in a business-oriented interface.

The improvement was assessed through demonstration with 2024 workflow data and stakeholder feedback. Within the evaluated scope, the prototype made intermediate stages more visible, reduced ambiguity in result checking, and supported more practical validation of derived outputs. Stakeholder feedback indicated that the separated workflow views helped users follow dependencies and inspect intermediate results more directly than in the SAP-based workflow.

More broadly, the Volvo CE case suggests that inspection-oriented representations can add value in ERP-based enterprise workflows where business understanding depends on seeing how staged transformations connect inputs, rules, and outputs rather than only reviewing final reported values.
...
%     \item \textbf{Sub-RQ2:} How can the workflow stages of preparation, market-view derivation, adjustment, machine line split, and resplit be represented in a web-based system so that users can more clearly follow how results are derived from CRP, SAL, OTH, and related business rules?
...
% A key contribution is that the artefact exposes intermediate workflow stages rather than only final aggregated outputs. Through dedicated views for preparation, market-view derivation, adjustment, machine line split, and resplit-related results, users can inspect how values are formed step by step, compare grouped results with source rows, and validate whether the implemented logic behaves as expected. Filterable tables, summary values, charts, and CSV export further support both overview and row-level inspection within the same interface.
...
% A key contribution is that the prototype exposes intermediate workflow outputs, including preparation, market-view derivation, adjustment, machine line split, and resplit-related results, rather than only presenting final aggregated figures. Through filterable tables, summary values, charts, and CSV export, users can inspect how values are formed, compare outputs with source data, and validate whether the implemented logic behaves as expected.
...
The main practical contribution of this thesis is a case-grounded demonstration of how selected parts of an SAP-based enterprise workflow can be re-expressed in a user-oriented environment for validation, inspection, and communication without replacing the underlying operational platform. The implemented prototype for selected stages of the Volvo CE Total Market Calculation (TMC) workflow serves as the concrete instantiation of that contribution. Its originality lies in the independent design and development of a dedicated inspection-oriented solution for the studied Volvo CE context, where no such representation previously existed.

A key contribution is the exposure of intermediate workflow outputs, including preparation, market-view derivation, adjustment, machine line split, and resplit-related results, instead of only final aggregated figures. Through filterable tables, summaries, charts, and CSV export, users can inspect how values are formed and compare outputs with source data. The contribution is therefore not a new algorithm, but a case-grounded design contribution that shows how enterprise workflows with heterogeneous inputs, embedded rules, and sequential transformations can be made more inspectable through structured input management, staged output exposure, and workflow-oriented review views.


\subsection{Academic Contributions}

On the academic side, this thesis contributes a concrete example of how transparency, traceability, usability, and inspectability challenges in complex enterprise data-processing workflows can be addressed through a workflow-oriented web-based artefact. While the implemented prototype is grounded in the Volvo CE TMC case, the underlying design problem is not limited to that setting. Many enterprise workflows depend on heterogeneous inputs and staged transformations, and their intermediate results may be difficult for business users to inspect and validate \cite{foidl2024pipeline,putrama2024heterogeneous}.

Prior research has shown that enterprise data processing is often organized as staged ETL or decision-support workflows \cite{vassiliadis2009etl}. ERP usability also affects how such systems are experienced by end users \cite{calisir2004erp}. In addition, embedded technical logic is often difficult to communicate and validate from a business perspective \cite{weber2022sap}.

Against this background, the academic contribution of this thesis is not a new algorithm, but design knowledge about how selected enterprise workflow logic can be re-expressed in a more reviewable and business-facing form. More specifically, the thesis shows how structured input handling, staged output exposure, interactive inspection, and explicit intermediate representations can support validation, communication, and error investigation in complex enterprise data-processing workflows. In this sense, the thesis contributes tentative transferable design principles for making embedded workflow logic more inspectable across similar ERP-based enterprise settings, even though the present evaluation remains grounded in one case.

Methodologically, the thesis also provides a Design Science Research example that uses a case study setting to translate a practical enterprise workflow problem into design requirements, iterative artefact development, and formative evaluation \cite{johannesson2021}. In this way, the Volvo CE case functions as the empirical setting through which a broader workflow-design problem is investigated rather than as the sole relevance of the study.
