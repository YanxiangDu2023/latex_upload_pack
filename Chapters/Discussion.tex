...
The main practical contribution of this thesis is a working web-based prototype for selected stages of the Volvo CE Total Market Calculation (TMC) workflow. Rather than replacing SAP, the artefact demonstrates how selected parts of an SAP-based enterprise workflow can be re-expressed in a user-oriented environment for validation, inspection, and communication.

A key contribution is the exposure of intermediate workflow outputs, including preparation, adjustment, and split-related results, instead of only final aggregated figures. Through filterable tables, summaries, charts, and CSV export, users can inspect how values are formed and compare outputs with source data. More broadly, the case contributes practical design knowledge on how enterprise data-processing workflows with heterogeneous inputs, embedded rules, and sequential transformations can be made more inspectable through staged output exposure and workflow-oriented review views.
...
Future work falls into three main directions. First, the artefact can be extended beyond the currently implemented stages to cover more of the TMC process and move closer to an end-to-end workflow representation. Second, more rules and mappings could be moved from backend code into configurable structures or administrative interfaces, reducing the need for direct code changes. Third, broader internal use would require stronger deployment, scalability, monitoring, and integration support, together with richer investigation features such as deeper drill-down and cross-view comparison.
