...
Sub-RQ1 was answered by turning file handling into an explicit part of the prototype rather than treating it as a separate preparatory task. In the implemented system, source datasets and matrix files have dedicated upload and management views, and each file is validated, normalized, and stored with metadata such as type, timestamp, row count, processing status, and version information. This matters in the Volvo CE case because later stages depend on knowing exactly which source file or matrix version was used.

Users can inspect rows in the interface, download stored files, save edited content as a new version, and check whether the required inputs are present before running downstream stages. In the 2024 evaluation, the prototype handled different source and matrix files correctly across upload, storage, retrieval, download, and re-save operations. Within the Volvo CE case, this shows that heterogeneous inputs can be managed in a more controlled and inspectable way than ad hoc file exchange. It also shows that input management is not merely a supporting utility around the workflow: when matrix versions and uploaded source files matter for later interpretation, file handling itself becomes part of transparency and traceability.
...
The main change was reviewability. In preparation, users can see how CRP, SAL, and OTH records are aligned with mappings and control fields before later calculations continue. In market view and adjustment, they can inspect how prepared data is reorganized and how source-detail and derived rows relate. In machine line split and resplit, the prototype exposes split ratios, before-and-after totals, and redistributed rows. Pipeline Viewer and Layer Detail pages tie these stages together. In the case setting, this changed the discussion from "what final figure did the process produce?" to "which stage produced this value, under which rule context, and from which prepared data?" More broadly, this shows one way of re-expressing ERP-based workflow logic so that business users can inspect the transformation path instead of treating it as a black box.
...
The prototype improved that situation by putting the selected stages into one inspection-oriented environment. Upload and matrix-management pages made input handling easier to review, Pipeline Viewer made stage dependencies visible, and Layer Detail pages exposed row-level outputs, summaries, and rule-related fields. Stakeholder feedback pointed in the same direction: the separated stage views made the process easier to explain and check than the previous SAP-based way of inspecting the same scope. The improvement should therefore be understood less as a claim that the prototype is universally "better than SAP" and more as evidence that inspection-oriented representations can add value when business users need to see how inputs, maintained rules, and staged transformations connect to the reported result.
...
% From an academic perspective, this thesis provides a practical example of how a complex enterprise data-processing setting can be re-expressed in a more inspectable and user-oriented prototype environment. By applying the Design Science Research approach, the study translates a problem of limited transparency in a SAP-based process into design requirements, artefact development, and evaluation.
...
The main practical contribution of this thesis is the development of a working prototype that lets users inspect selected TMC stages in a way that was not previously available in the Volvo CE setting. Rather than replacing SAP, the prototype complements the existing implementation with an environment for upload review, staged inspection, and discussion of intermediate results. In practical terms, this means that business users gain one place where they can review what was uploaded, which stage was executed, what intermediate values were produced, and where a mismatch becomes visible. Subject to further development, full testing, and internal infrastructure integration, the prototype could potentially support internal use in a future TMC cycle.

Its originality lies in how the selected stages were re-expressed for inspection. Preparation, market view, adjustment, machine line split, and resplit are shown as explicit reviewable stages, and users can work with filterable tables, summaries, charts, and CSV export instead of relying only on final aggregated outputs or technical SAP structures. The contribution is therefore a concrete design contribution rather than a new algorithm.

\subsection{Academic Contributions}
On the academic side, the thesis contributes a concrete example of how transparency, traceability, usability, and inspectability problems in ERP-based enterprise data processing can be addressed through an implemented artefact rather than only discussed conceptually. The Volvo CE case is specific, but the underlying issue is wider. Prior work on enterprise data pipelines and heterogeneous data integration shows that many such workflows depend on staged transformations and the combination of different input structures \cite{vassiliadis2009etl,putrama2024heterogeneous}. In SAP-related settings, embedded rules and system-specific structures can then make these workflows difficult for business users to inspect \cite{weber2022sap}. Usability concerns in ERP environments have also been reported in prior research \cite{calisir2004erp}.

The academic contribution is therefore best understood as design knowledge, not as a new algorithm. In this case, the study shows that structured input management, explicit intermediate outputs, and stage-oriented inspection views can support validation, explanation, and error investigation in workflows that would otherwise be difficult to follow from a business-user perspective. More specifically, the case suggests that inspection value does not come from visualization alone, but from the combination of persisted staged outputs, visible rule context, and navigation between workflow status and row-level detail. The transferable value lies in that design approach, even though the present evidence still comes from one case.

Methodologically, the thesis also shows how a DSR project can stay grounded in a real organizational workflow while still moving beyond pure description. The Volvo CE case was used to identify the problem, clarify requirements, guide implementation choices, and evaluate the resulting artefact \cite{johannesson2021}.
\section{Limitations}
...
Finally, the prototype has not yet been evaluated for production-level deployment or large-scale multi-user use. Broader internal use would require enterprise-grade authentication, role-based access control, monitoring, backup and recovery, process orchestration, and deployment within Volvo CE's internal infrastructure.


\section{Ethical and Societal Implications}

The artefact provides a more accessible way to process and inspect selected parts of the Volvo CE TMC process. The main ethical considerations concern responsible handling of company-internal data and avoiding overreliance on generated outputs. Since the results still depend on uploaded data, mappings, and implemented rules, the artefact should be treated as a validation and inspection tool rather than as a guarantee of correctness. Prior work on automation bias and algorithmic accountability likewise suggests that decision-support systems should remain open to human review rather than being treated as unquestionable sources of truth \cite{parasuraman1997humans,martin2019ethics}. Future use should therefore keep intermediate steps and assumptions visible so that errors can be traced and reviewed rather than hidden behind final outputs.

Beyond the immediate Volvo CE case, inspection-oriented designs may also improve accountability in enterprise reporting and planning by making workflow stages easier to review and discuss. At the same time, greater visibility does not automatically remove organizational risks: if introduced without care, such systems could also concentrate process knowledge in a new technical layer or be used in ways that increase managerial control over workflow activities. Their broader value therefore depends not only on technical transparency, but also on how they are governed and used in practice.
...
Future work should address both system scope and research scope. A first step would be to extend the artefact to later workflow stages such as full Total Market Calculation, Restatement, and Reporting, so that a larger part of the workflow can be inspected in one environment. That would make it possible to follow how early preparation and redistribution decisions influence later market-share outputs rather than stopping at intermediate stages only.

A second direction concerns configurability and traceability. More mappings, workflow conditions, and calculation rules could be moved out of backend code and into configurable structures that are easier to review and maintain over time. Further work could also strengthen drill-down from outputs to original inputs, make rule applications more explicit, preserve clearer version history for source and matrix changes, and support comparison between workflow runs or yearly cycles. Together, these additions would make the artefact more useful not only for review, but also for investigation and change analysis.

A third direction concerns deployment readiness. Broader internal use would require stronger operational capabilities such as enterprise-grade authentication, role-based access control, workflow orchestration, monitoring, backup and recovery, and deployment within Volvo CE's internal infrastructure. If those capabilities were added, the prototype could be evaluated not only as a research artefact, but also as a more mature internal support tool.

The artefact should also be studied over a longer period in regular organizational use and with additional yearly datasets. This would make it possible to assess whether stage-oriented inspection actually reduces validation effort, improves communication between business and IT, and changes how users interpret unexpected outputs in recurring workflow cycles.

Finally, future research should examine transferability beyond the Volvo CE case by applying the same inspection-oriented design approach in another enterprise data-processing setting with similar characteristics, such as heterogeneous inputs, maintained rules, and staged transformations. This would help distinguish more clearly between case-specific findings and more transferable design principles.
