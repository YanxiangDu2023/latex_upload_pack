...
This chapter outlines the background needed to understand the thesis problem. It introduces the industry setting, explains why market share estimation becomes a complex enterprise workflow, summarises how that workflow is implemented in the Volvo CE case, and reviews related work relevant to transparency, traceability, and usability in enterprise analytics.
...
The construction equipment industry is global, capital-intensive, and analytically segmented by country, machine line, size class, and brand \cite{cece2025,volvo2024annual,mordor2024construction,offhighway2024sample}. For manufacturers, this means that market analysis depends on integrating heterogeneous sources and maintaining consistent classifications across many dimensions. This thesis focuses on that manufacturer perspective, specifically on how firms estimate total market size and market share across regions and product categories.
...
Given the competitive and international nature of the industry, market share is a key indicator for assessing position and supporting strategic decision-making \cite{porter1980competitive, kotler2016marketing}. Yet market size and market share are rarely derived from one standardized source, so firms must combine internal sales data, industry-level exchange or aggregated data, and external third-party inputs.
...
Because market share estimation involves integrating heterogeneous data and applying repeatable rules across multiple business dimensions, it is typically implemented within enterprise systems rather than isolated spreadsheets or ad hoc scripts \cite{klaus2000}. In the Volvo CE case, this is realised through a Total Market Calculation (TMC) framework implemented in SAP for market reporting and market share analysis.
...
Within the SAP landscape, transactional processing and analytical processing are separated. Operational data is generated in ERP systems, while SAP Business Warehouse (BW) provides the analytical layer for reporting, modelling, and transformation \cite{sap_bw_guide}. For this thesis, the key point is not SAP internals in themselves, but that SAP combines structured data loading, multidimensional storage, sequential rule execution, and spreadsheet-based user interaction \cite{sap_enhanced_star_schema,sap_bw_ip_overview,sap_planning_sequences,sap_analysis_office_guide}. Together, these capabilities form the technical foundation for a controlled enterprise workflow, while also making the calculation logic difficult to inspect from a business-user perspective.
...
Overall, the SAP-based TMC framework can be understood as a structured enterprise workflow that integrates maintained rules, staged transformation, and reporting in one operational environment.
...
Prior research points in three directions relevant to this thesis. First, data-warehouse and ETL literature describes complex analytics as staged workflows that integrate heterogeneous data and therefore benefit from explicit transformation steps and quality control \cite{march2007dss,vassiliadis2009etl,souibgui2019etl}. Second, enterprise-system usability research shows that difficult interfaces and technically fragmented system structures reduce end-user effectiveness, especially for non-technical business users working with ERP-based analytics \cite{calisir2004erp,wong2016sap,balaha2025integration}. Third, work in the construction domain shows that analytical functionality can be delivered through separate decision-support layers rather than only through transactional enterprise systems \cite{fan2004webdw,fan2006equipmentdw}.

Taken together, this literature supports the idea that enterprise data-processing workflows benefit from explicit stages, more accessible review interfaces, and clearer exposure of intermediate outputs. However, the reviewed work does not provide a domain-specific design for reconstructing SAP-embedded market-share logic into a transparent web-based inspection workflow. This thesis addresses that gap through the Volvo CE case.
