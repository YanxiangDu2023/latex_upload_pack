...
Enterprise resource planning (ERP) systems are widely used to integrate and manage core business processes across large organizations \cite{davenport1998,klaus2000}. Rather than treating finance, operations, procurement, manufacturing, logistics, and sales as isolated activities, ERP systems provide a shared environment in which data and processes can be coordinated across functional boundaries \cite{sap_erp_guide}. ERP should therefore be understood not only as a transactional business system, but also as an enterprise data-processing environment in which reporting, planning, and calculation processes are executed.

Prior literature also shows that ERP environments support activities beyond transaction processing, including reporting, planning, performance monitoring, and analytical routines \cite{monk2012erp}. In this thesis, that supports treating complex business calculations in enterprise systems as part of a broader workflow context rather than as an isolated practice in one company. SAP is one of the most widely used ERP platforms in this landscape. Within SAP environments, workflow execution is often distributed across data-loading mechanisms, storage structures, transformation logic, reporting layers, and user-facing interfaces, which makes SAP a relevant example of the kind of embedded enterprise workflow studied here.
...
This thesis is grounded in the case of Volvo Construction Equipment (Volvo CE), whose market-analysis work is situated in the construction equipment industry. Industry reports describe a market that spans multiple countries, product segments, and regional conditions \cite{cece2025,mordor2024construction}. Because manufacturers operate across these dimensions, market analysis depends on combining inputs for total market size and market share across regions and product categories.
...
In this industry setting, market share is a key indicator for assessing position and supporting strategic decision-making \cite{kotler2016marketing}. Yet market size and market share are rarely derived from one standardized source, so firms must combine internal sales data, industry-level exchange or aggregated data, and external third-party inputs. Off-Highway Research, for example, describes the use of information from manufacturers, dealers, and regional sources in market-estimation work \cite{offhighway2018method}, and industry reports indicate that external sources are used when reporting coverage is incomplete \cite{mordor2024construction}.
...
In complex industrial sectors such as construction equipment, market analysis often requires the integration of heterogeneous data across countries, product segments, brands, and reporting dimensions. Because market share estimation involves combining such data and applying repeatable rules across multiple business dimensions, firms may choose to implement these workflows in enterprise systems rather than rely only on isolated spreadsheets or ad hoc scripts. To understand and analyse how such ERP-based enterprise data-processing challenges appear in practice, this thesis uses Volvo CE as a case study. In the Volvo CE case, this is realized through a Total Market Calculation (TMC) framework implemented in SAP for market reporting and market share analysis.
