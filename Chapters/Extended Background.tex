...
Overall, the SAP-based TMC implementation can be understood as one example of a broader class of ERP-based enterprise workflows that integrate maintained rules, staged transformation, and reporting in one operational environment.
...
Although the existing SAP-based TMC implementation provides a structured environment for market share calculation, it introduces several limitations:
...
These limitations motivate the need for a more transparent and traceable representation of the TMC process, which forms the starting point of this thesis.
...
Taken together, this literature supports the problem framing used in this thesis: enterprise workflows can be technically robust and operationally useful while still being difficult for business users to inspect step by step. At the same time, the reviewed work does not provide a concrete browser-based design example that re-expresses selected SAP-based workflow logic as explicit, reviewable stages for business users. This thesis addresses that gap through a Design Science Research study using Volvo CE's Total Market Calculation process as the empirical case setting, while framing the underlying problem as a broader enterprise workflow challenge.
