...
This chapter provides the background for the thesis. It first introduces ERP systems and SAP as a general enterprise workflow environment, then presents the industry and Volvo CE case context, and finally reviews related work relevant to transparency, traceability, and usability in enterprise analytics.


\section{ERP Systems, SAP, and Enterprise Workflows}

Enterprise resource planning (ERP) systems are widely used to integrate and manage core business processes across large organizations \cite{davenport1998,klaus2000}. Rather than treating finance, operations, procurement, manufacturing, logistics, and sales as isolated activities, ERP systems provide a shared environment in which data and processes can be coordinated across functional boundaries \cite{sap_erp_guide}. In that sense, ERP should be understood not only as a transactional business system, but also as an enterprise workflow environment in which important reporting, planning, and calculation processes are executed.

Prior literature also shows that ERP-based environments are routinely used beyond basic transaction processing. Integrated enterprise data is reused for managerial reporting, planning, performance monitoring, and domain-specific analytical routines, often across several organizational units \cite{monk2012erp,fan2006equipmentdw}. This suggests that complex business calculations embedded in enterprise systems should be understood as part of a broader enterprise workflow pattern rather than as an isolated practice in one company.
...
This complexity is discussed in prior work. Research on SAP ERP process extraction shows that the amount of processes and data contained in SAP ERP is enormous, making the identification of relevant process structures and related tables a significant challenge \cite{weber2022sap}. ERP usability studies further show that users may experience difficulties related to navigation, task support, learnability, and presentation of outputs \cite{calisir2004erp,wong2016sap}. These observations support the idea that transparency, traceability, and usability problems are not unique to one company-specific workflow, but are relevant to ERP-based enterprise workflows more broadly.
...
This thesis is grounded in the case of Volvo Construction Equipment (Volvo CE), whose market-analysis work is situated in the construction equipment industry. The construction equipment industry is global and capital-intensive \cite{cece2025,mordor2024construction}. Manufacturers operate across countries, product categories, and customer segments, so market analysis depends on combining inputs across several business dimensions, including the estimation of total market size and market share across regions and product categories.





% Market share is a widely used indicator to evaluate a firmâ€™s competitive position within a defined market. It is typically defined as the ratio between a companyâ€™s sales and the total market size within a given segment \cite{kotler2016marketing}. In practice, business units within companies estimate both the total market size and their own share in order to assess competitive dynamics, benchmark performance against competitors, and support strategic planning and decision-making \cite{porter2008competitive}.
...
% % As Vassiliadis et al. \cite{vassiliadis2002etl} argue, complex analytical results are generally produced through multi-step transformation workflows rather than isolated functions. Market share calculation in SAP can therefore be understood as one such workflow, implemented on top of SAPâ€™s analytical infrastructure.
...
In the Volvo CE setting, SAP provides the technical environment in which the TMC workflow is executed. Operational data is generated in enterprise systems, while SAP Business Warehouse (BW) provides the analytical layer for reporting, modeling, and transformation \cite{sap_bw_guide}. For this thesis, the key point is that SAP combines structured data loading, sequential rule execution, and spreadsheet-based user interaction \cite{sap_bw_ip_overview}. In the Volvo CE case, these capabilities form the technical foundation for a controlled enterprise workflow, while also making the calculation logic difficult to inspect from a business-user perspective.
...
At Volvo CE, market share estimation is implemented through an SAP-based TMC framework that combines maintained configuration data with several categories of workflow input, especially internal sales data (SAL), industry exchange data (CRP), derived total-market reference data (TMA), and supplementary external data (OTH). The workflow is staged rather than monolithic: master data and matrix rules are maintained first, source data is then loaded into BW structures, and calculation logic is executed through planning functions and planning sequences.

For the purposes of this thesis, three aspects of that implementation are most important. First, the workflow depends on maintained classification structures and mapping rules, not only on raw source files. Second, calculation is executed through sequential preparation, transformation, redistribution, and reporting stages. Third, the resulting logic is distributed across several SAP artefacts and interfaces rather than exposed as one inspectable workflow view. These characteristics make the TMC process a relevant case for studying transparency and traceability in enterprise data-processing workflows.
...
Overall, the SAP-based TMC framework can be understood as one example of a broader class of ERP-based enterprise workflows that integrate maintained rules, staged transformation, and reporting in one operational environment.
...
\begin{itemize}
    \item \textbf{Technical complexity:} The calculation logic is implemented through SAP-specific objects such as planning functions, planning sequences, and ABAP-based processing, which require specialized technical knowledge and limit accessibility for business users \cite{weber2022sap}.
...
Prior research relevant to this thesis can be grouped into three areas. First, data-warehouse and ETL literature describes enterprise analytics as staged workflows in which heterogeneous data is extracted, transformed, and prepared for reporting or decision support \cite{vassiliadis2009etl}. Related work on heterogeneous data integration and data-pipeline quality further shows that such workflows often involve inconsistent inputs, transformation errors, and data-quality challenges \cite{putrama2024heterogeneous,foidl2024pipeline}. This supports the need for explicit transformation stages and quality-control mechanisms in complex enterprise data-processing workflows.

Second, ERP and SAP-related research highlights the complexity and usability challenges of enterprise systems. ERP systems integrate business processes across organisational functions, but this integration can also introduce complex data structures, system-specific logic, and interfaces that require specialised knowledge \cite{davenport1998,klaus2000}. Research on SAP ERP process extraction shows that identifying relevant process structures and tables can be difficult in SAP environments \cite{weber2022sap}. ERP usability studies also show that navigation, task support, learnability, and output presentation affect end-user effectiveness \cite{calisir2004erp,wong2016sap}. These findings are relevant because the problem addressed in this thesis concerns not only workflow execution, but also whether business users can inspect and validate how outputs are produced.
...
Taken together, this literature supports the relevance of transparency, traceability, usability, and inspectability in enterprise data-processing workflows. However, the reviewed work does not provide a concrete design example of how selected ERP/SAP-embedded workflow logic can be re-expressed as a staged, browser-based inspection artefact for business users. This thesis addresses that gap through a Design Science Research study using Volvo CE's Total Market Calculation workflow as the empirical case setting, while framing the underlying problem as a broader enterprise workflow challenge.
