...
This chapter provides the background necessary to understand the context and focus of the thesis. It begins with an overview of the construction equipment industry and the role of market share estimation within it, before examining how such estimation is implemented at the enterprise level through SAP-based systems. The chapter then describes the existing TMC framework at Volvo CE and its limitations, and concludes with a review of related work that motivates the design approach adopted in this thesis.




\section{Construction Equipment Industry Overview}
...
The construction equipment industry is a global and capital-intensive sector that supports infrastructure development, mining, and urbanization. The sector includes internationally active manufacturers such as Volvo Construction Equipment, Caterpillar, Komatsu, Hitachi Construction Machinery, and Liebherr, and is characterized by operations across multiple regional markets \cite{cece2025,volvo2024annual}.

Construction equipment is commonly categorized into machine lines based on functionality. Core categories include hydraulic excavators, wheel loaders, articulated and rigid haulers, bulldozers, and road construction equipment. These machine lines correspond to different operational tasks, such as excavation, loading, transportation, and surface preparation. Within each machine line, equipment may be further segmented into size classes, often based on operating weight ranges, reflecting differences in capacity, application scenarios, and customer requirements \cite{mordor2024construction}.

From an industry perspective, construction equipment markets are commonly analysed across dimensions such as country or region, machine line, size class, and brand. These dimensions are used in industry reporting, equipment analysis, and regional market statistics \cite{cece2025,offhighway2024sample}. The industry also relies on a broad commercial ecosystem that includes manufacturers, distributors, dealers, rental companies, and service providers. Through these networks, equipment is sold and supported across a large number of countries, while market information is documented through company reports, industry statistics, and specialized market intelligence publications \cite{offhighway2024sample,aem2020impact}.

While the construction equipment ecosystem involves multiple stakeholders, this study focuses specifically on the perspective of equipment manufacturers. In particular, it examines how manufacturers evaluate their market position by estimating total market size and market share across different regions, machine lines, and product segments.




\section{Market Share Estimation in the Construction Equipment Industry and Associated Challenges}
...
Given the global and highly competitive nature of the construction equipment industry, manufacturers must continuously evaluate their performance across regions, product segments, and competitors. In this context, market share serves as a key indicator for assessing competitive position and supporting strategic decision-making \cite{porter1980competitive, kotler2016marketing}. 

However, there is no universally standardized methodology for calculating total market size and market share. Due to fragmented data availability and inconsistent reporting practices across the industry, companies must estimate the total market by integrating multiple data sources and applying internal assumptions and transformation rules.

In practice, market share estimation relies on combining three main types of data: internal company sales data, industry-level aggregated or exchange data, and external third-party datasets. Industry intelligence providers, such as Off-Highway Research, aggregate data from manufacturers, dealers, and regional sources to construct market-level estimates \cite{offhighway2018method}. To address gaps in reporting coverage, additional external inputs—such as dealer estimates, registration data, and country-level statistics—are often incorporated \cite{mordor2024construction}.
...
These challenges motivate the need for structured data processing frameworks that can integrate heterogeneous data sources while ensuring consistency and reliability in market share estimation.
...
% % As Davenport \cite{davenport1998} and Klaus et al. \cite{klaus2000} point out, enterprise resource planning (ERP) systems are designed to integrate core business processes and consolidate operational data across organizational functions. Among these systems, SAP is one of the most widely adopted enterprise platforms, supporting business operations while also providing real-time data visibility and analytical capabilities \cite{sap_erp_guide}. 
...
Because market share estimation involves integrating heterogeneous data sources and applying repeatable transformation rules across multiple business dimensions, it is typically implemented within enterprise systems rather than isolated spreadsheets or ad hoc scripts \cite{klaus2000}. Among such systems, SAP is one of the most widely used ERP platforms and provides an environment for both data integration and analytical processing \cite{monk2012erp, sap_erp_guide}. At Volvo CE, this is realized through a Total Market Calculation (TMC) framework implemented in SAP, which supports market reporting and market share analysis.
...
% Together, these capabilities provide the technical basis for implementing market share calculation in a structured and scalable enterprise environment. In the case of Volvo CE, they support the TMC framework as a controlled enterprise workflow for market-related data integration and calculation.
...
% Together, these capabilities form a layered analytical environment in which data integration, storage, and rule-based processing are tightly coupled. This provides the technical foundation for implementing structured enterprise workflows.



Within enterprise systems, analytical processing is typically separated from transactional operations. In the SAP landscape, operational data is generated and consolidated in ERP systems, while SAP Business Warehouse (BW) provides the analytical layer for reporting, modeling, and data transformation \cite{sap_bw_guide}. This separation enables complex analytical workflows to be built on top of integrated enterprise data.
...
\subsection{The Existing TMC Framework and Its SAP-based Execution}
\label{subsec:tmc_framework_execution}
...
At Volvo CE, market share estimation is implemented through an existing Total Market Calculation (TMC) framework within an SAP Business Warehouse (BW) environment. The framework operates as a staged analytical workflow supported by several SAP technologies. Its main structure can be summarized as follows:

\begin{itemize}
    \item \textbf{Master data maintenance:} Before calculation begins, supporting master data and configuration structures are maintained in the system. These include matrices and classification rules related to brand information, country grouping, source definitions, participation settings, and size-class handling. In practice, these structures are maintained through SAP master data maintenance interfaces and Analysis for Office workbooks, which provide spreadsheet-based front ends for updating configuration-related content.

    \item \textbf{Source data integration:} The framework combines several main categories of market-related data:
\begin{itemize}
    \item \textbf{SAL}: Volvo CE's internal sales data, representing the company's own recorded sales volumes.

    \item \textbf{CRP}: Industry exchange data derived from standardized reporting systems, representing transaction-level market data contributed by participating entities within the industry.

    \item \textbf{TMA}: A total market reference measure derived within the SAP framework based on industry exchange data, used as a baseline for subsequent calculations.

    \item \textbf{OTH}: Additional external data outside the standard industry reporting scope, such as dealer estimates or supplementary market inputs, used to improve market coverage where reporting data is incomplete.
\end{itemize}

    \item \textbf{Data upload and storage:} External inputs are uploaded through controlled SAP loading mechanisms, including Analysis for Office templates, upload interfaces, and process chains. Once loaded, the data is stored in SAP BW analytical structures such as InfoProviders and planning cubes, where key figures and characteristics can be managed in a multidimensional format.

 \item \textbf{Calculation execution:} The execution logic of the TMC framework is implemented through SAP planning functions and planning sequences. The calculation process is structured into multiple stages:

\begin{itemize}
    \item \textbf{Preparation:} Initial steps standardize and align data from different sources, ensuring consistency across dimensions such as country, machine line, and size class.

    \item \textbf{Transformation and redistribution:} Subsequent steps apply rule-based transformations to the data. This includes redistribution logic such as machine line split operations, where data reported at a more aggregated level is reallocated into finer-grained segments.

    \item \textbf{Reference-based allocation:} The redistribution is guided by reference distributions derived from existing market structures, ensuring that the transformed data remains consistent with overall market patterns.
\end{itemize}

These staged transformations are executed sequentially within SAP, forming a controlled calculation workflow.

\item \textbf{Validation and reporting:} After the staged calculations are completed, the results are validated and transferred to reporting outputs. SAP reporting structures make the processed data available for downstream market analysis and market share reporting.
\end{itemize}
...
Overall, the SAP-based TMC framework can be understood as a structured workflow that integrates master data maintenance, data input, multidimensional storage, and planning-based calculation. Through the use of SAP BW components such as Analysis for Office, process chains, InfoProviders, and planning functions, the system supports controlled and consistent market share estimation in an enterprise environment.
...
\subsection{Limitations of the Current SAP-based Implementation}
\label{subsec:tmc_limitations}

Although the existing SAP-based TMC framework provides a structured environment for market share calculation, it introduces several limitations:

\begin{itemize}
    \item \textbf{Technical complexity:} The calculation logic is implemented through SAP-specific objects such as planning functions, planning sequences, and ABAP-based processing, which require specialized technical knowledge and limit accessibility for business users \cite{klaus2000}.

    \item \textbf{Limited transparency:} The calculation is executed across multiple system layers without a clear, consolidated representation of the full workflow, making intermediate transformations difficult to inspect and interpret.

    \item \textbf{Dependency on IT:} Modifying or validating business logic typically requires technical intervention within SAP, reducing flexibility and limiting the ability of business users to directly interact with the calculation process.

    \item \textbf{Reduced traceability:} The distributed implementation across multiple SAP objects makes it difficult to trace data transformations, verify intermediate results, and assess the impact of logic changes.
\end{itemize}

These limitations motivate the need for a more transparent and traceable representation of the TMC workflow, which forms the starting point of this thesis.
...
The complexity and limited transparency of enterprise analytical implementations have motivated prior research to explore more accessible and inspectable approaches to data processing and decision support. Although systems such as SAP provide strong capabilities for structured execution and data integration, analytical logic embedded in system-specific objects can be difficult for business users to follow, validate, or adapt. Prior work has therefore examined several directions relevant to this challenge, including data warehousing and ETL-oriented architectures, enterprise system usability, and web-based decision support in industrial contexts.

One important direction is the use of data warehouses and ETL-oriented workflows. March and Hevner describe data warehousing as a layered decision-support architecture in which heterogeneous internal and external data are integrated into a common analytical repository for management decision-making \cite{march2007dss}. Similarly, Vassiliadis characterizes ETL processes as the core mechanism for extracting, transforming, cleaning, and loading heterogeneous source data into analytical structures, thereby making multi-source analytical processing more explicit and manageable \cite{vassiliadis2009etl}. Souibgui et al. further highlight that ensuring data quality across ETL pipelines requires tracking transformations step by step, as defects introduced at one stage can propagate silently through subsequent processing \cite{souibgui2019etl}. From this perspective, complex calculations are represented as staged data workflows rather than as opaque system-internal functions.


A second direction concerns the usability of enterprise systems for business users. Prior research shows that interface usability, perceived usefulness, and ease of use are closely related to end-user satisfaction with ERP systems, and that systems perceived as difficult to use can reduce the effectiveness of analytical work \cite{calisir2004erp}. Studies of SAP use also indicate that complex system structures may create usability barriers for non-technical users, especially when users need to navigate technical objects or interpret system outputs \cite{wong2016sap}. More recent work on data integration for decision support further highlights that ERP-centered and smart manufacturing environments depend on effective data integration, while complexity and interoperability challenges can still limit the accessibility and effectiveness of analytical solutions \cite{balaha2025integration}.



Related work in the construction domain points in a similar direction. Fan and Kim developed a web-enabled data warehouse for decision support in construction equipment management, showing how analytical data can be delivered through a web application layer rather than relying solely on transactional systems \cite{fan2004webdw}. Fan et al. further proposed a data warehousing approach that integrates heterogeneous operational data and supports multidimensional, interactive analysis for equipment managers \cite{fan2006equipmentdw}. Although these studies are not focused on market share estimation, they demonstrate that construction-related analytics can benefit from separating analytical workloads from operational systems and exposing them through more accessible decision-support interfaces.


However, the reviewed literature does not provide a domain-specific solution for market share estimation in the construction equipment industry that reconstructs SAP-embedded calculation logic into a transparent analytical workflow with a dedicated web-based interaction layer. This thesis addresses that gap by designing such a prototype in the context of Volvo CE's Total Market Calculation process.