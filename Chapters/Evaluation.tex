...
After the test environment had been prepared, the prototype was executed with the selected 2024 TMC workflow data. The execution moved from file upload and backend processing to preparation, market view generation, adjustment, split and resplit processing, and output review.
...
The artefact was evaluated against the requirements defined in Section~\ref{sec:requirements}. The evaluation combined output validation with requirement-based assessment, focusing on whether generated results followed expected business rules and whether the prototype supported transparency, traceability, usability, validation support, and prototype-level performance.
...
The generated prototype outputs were validated against expected workflow logic and, where comparable, against 2024 SAP reference outputs. The focus was on whether the implemented processing produced correct results within the selected scope.


\subsection{Preparation-Stage Validation}

The preparation stage was validated through manual Excel-based checks, comparison with 2024 SAP reference data, and stakeholder clarification. CRP-related preparation matched the SAP reference result, including the identification of 338 reporter rows. For OTH-related preparation, mapped fields such as country, market area, machine line, brand code, and size class were manually checked and found to follow the intended rules.

The main discrepancy concerned OTH reporter-row counts: the prototype identified 3,452 reporter rows, while the SAP reference output contained 3,467 reporter rows. This corresponds to a row-count difference of 15 rows and a net FID difference of 68. Manual checking and later stakeholder clarification showed that the difference came from a reporter-list version adjustment rather than from incorrect prototype execution. Selected VCE / Non-VCE market view records were also spot-checked in Excel and were consistent with the expected calculations.
...
The split- and resplit outputs were validated through manual Excel-based checks, SAP reference comparison, and output inspection. Records that should enter the split process were manually filtered in Excel and compared with the prototype's split input totals. The totals were consistent for the tested Excavators and Wheel Loaders split cases, indicating correct record selection.

Approximately 10\% of split output rows and 10\% of resplit output rows were then spot-checked across selected country, source, machine-line, and size-class combinations. The sampled records followed the intended split and resplit logic, and the before/after totals remained consistent. Some differences from SAP outputs were observed, but these were treated as reference-data and rule-clarification issues rather than direct prototype failures.
...
Overall, the main requirements were met within the prototype scope. The system supported upload and reuse of the required datasets, generated staged outputs for the selected workflow layers, and provided structured inspection through filters, summaries, charts, and CSV export. Validation support was also largely confirmed: manually checked rule behavior matched the prototype outputs, CRP reporter rows matched the SAP reference result, and sampled market view and split and resplit records followed the expected calculation logic. The main observed exception was an OTH reporter-row difference that was traced to a reporter-list version difference rather than a prototype error. Repeated workflow cycles remained within the defined review window, and no crash, data loss, or inconsistent save/reload behaviour was observed during the repeated test runs.

\begin{table}[htbp]
    \centering
    \caption{Condensed evaluation summary for the implemented scope.}
    \label{tab:condensed_evaluation_summary}
    \setlength{\tabcolsep}{4pt}
    \small
    \begin{tabularx}{\textwidth}{|>{\raggedright\arraybackslash}p{3.4cm}|>{\raggedright\arraybackslash}p{2.3cm}|X|}
        \hline
        \textbf{Evaluation area} & \textbf{Outcome} & \textbf{Summary} \\
        \hline
        Functional support & Largely met & Upload, staged processing, and browser-based inspection were implemented and used successfully in the evaluated workflow scope. \\
        \hline
        Transparency and usability & Strongly supported & Intermediate outputs, filters, summaries, charts, and CSV export made selected stages easier to inspect and discuss. \\
        \hline
        Traceability and validation & Partly to strongly supported & Stage-level traceability and rule checking were supported well, but full end-to-end row-level lineage was not implemented. \\
        \hline
        Scope and deployment readiness & Partly addressed & The prototype worked well for the selected stages, but full TMC coverage and production-level integration remain future work. \\
        \hline
    \end{tabularx}
\end{table}

A more detailed requirement-by-requirement table is provided in Appendix~\ref{app:supplementary-material}.
...
\begin{table}[htbp]
    ...
    \setlength{\tabcolsep}{4pt}
    \small
    \begin{tabularx}{\textwidth}{|>{\raggedright\arraybackslash}p{2.1cm}|>{\raggedright\arraybackslash}p{1.8cm}|>{\raggedright\arraybackslash}p{1.8cm}|X|}
        \hline
        \textbf{Dimension} & \textbf{SAP-based Workflow} & \textbf{Prototype} & \textbf{Evidence in this thesis} \\
        \hline
        Transparency & Limited for business-user inspection & Supported & Preparation, market view, adjustment, machine line split, and resplit outputs are exposed as separate inspectable stages with row-level and summary views. \\
        \hline
        Traceability & Limited and dispersed across technical structures & Partly supported & Stage outputs preserve source and rule context fields, including country, machine line, source, brand, flags, split level, and split ratio, enabling stage-level tracing. \\
        \hline
        Usability & Requires more SAP-specific navigation & Supported & Browser-based filtering, summaries, charts, and CSV export supported practical BI review tasks. \\
        \hline
        Validation support & Often requires extra extraction or IT support & Supported & Intermediate values and rule-relevant fields were directly inspectable and comparable with expected business rules and SAP references. \\
        \hline
    \end{tabularx}
\end{table}

Compared with the SAP-based representation, the prototype made four differences visible within the evaluated scope: more explicit stage transparency, better stage-level traceability through preserved source and rule context, easier business-user review through browser-based inspection functions, and more direct validation of intermediate values without additional extraction or IT-supported investigation.
