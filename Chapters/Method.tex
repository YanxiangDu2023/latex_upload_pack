...
Design Science Research (DSR) was selected because this thesis was not limited to describing the existing SAP-based TMC implementation. The main task was to build an artefact, use it on realistic case material, and evaluate whether it made the selected process stages easier to inspect \cite{johannesson2021,hevner2004design}. In DSR terms, the artefact is the proposed inspection-oriented solution, while the implemented prototype is its concrete instantiation in this thesis. For that reason, DSR matched the project better than a purely descriptive research strategy.

The work followed the five DSR activities described by Johannesson and Perjons \cite{johannesson2021}, but not as a rigid linear sequence. Early problem understanding, requirement clarification, implementation, and evaluation informed each other during the project. Some parts of the workflow could be understood from documents and source structures, but other rule details had to be clarified with stakeholders before the prototype behaviour could be implemented with confidence.

This thesis combines Design Science Research (DSR) with a case study. \textbf{Alternative research methods.} A stand-alone \textbf{case study} was less suitable because the thesis was not only about describing how the existing process works, but also about designing and evaluating an alternative representation of selected stages \cite{yin2018}. \textbf{Action research} was also less suitable, since the project did not aim to intervene directly in Volvo CE's live production process or run an organizational change initiative \cite{susman1978}. More measurement-oriented approaches, such as \textbf{surveys} or \textbf{controlled experiments}, were likewise less suitable. The work depended on a small group of knowledgeable stakeholders and on detailed interpretation of workflow logic rather than on broad population-level comparison.

At the same time, the research clearly used the Volvo CE case as its empirical grounding. The broader design problem concerns transparency, traceability, and usability in ERP-based enterprise data processing, but that problem was studied through one concrete setting: the TMC process at Volvo CE. Within the overall DSR design, the Volvo CE setting functioned as a case study with descriptive and evaluative elements. It was used to understand the workflow problem in context, guide artefact design, and assess the implemented prototype in a real organizational environment rather than to test causal explanations across multiple cases. In this thesis, DSR provides the overall research logic, while the case study provides the practical context in which the problem, artefact, and evaluation can be understood.
...
This thesis used three main sources of empirical material: \textbf{document analysis}, \textbf{stakeholder discussions}, and \textbf{prototype-based material}. The choice was driven by the nature of the case. To understand the TMC process, it was necessary to review workflow descriptions, source files, and matrix structures, but that was not enough on its own because some rule interpretations still depended on business context.

Document analysis was therefore used to examine process materials, workflow descriptions, source data structures, and matrix files \cite{bowen2009document}. The document set was limited to materials directly relevant to the studied 2024 TMC workflow and the implemented stages, especially documents and files used operationally to describe source formats, matrix logic, workflow steps, and existing outputs. Outdated versions, unrelated SAP reports, and partial notes that could not be connected to the selected workflow scope were not treated as primary analytical material. Document credibility was judged through source authority, operational relevance, recency, and consistency with stakeholder explanations, source structures, and observed system outputs. When documents conflicted with each other or left rule details unclear, stakeholder clarification and output comparison were used to resolve the interpretation.

Stakeholder discussions and weekly review meetings were used to clarify business rules, expected outputs, and places where the available documentation was incomplete or difficult to interpret \cite{kallio2016systematic}. Prototype-based material was then collected during design, demonstration, and evaluation in the form of uploaded inputs, generated intermediate outputs, result comparisons, and stakeholder comments on the behaviour of the prototype. This combination fits a DSR study in a real organizational setting and is also consistent with case-based software research \cite{runeson2009guidelines}.
...
The fourth DSR activity was to demonstrate the artefact. This activity shows how the artefact can be applied in a relevant case and can serve as a proof of feasibility for the proposed solution \cite{johannesson2021,hevner2004design}. In this thesis, the artefact was demonstrated through the Volvo CE case study, using a realistic TMC scenario with Volvo CE's 2024 market data and matrix files used for the yearly TMC calculation.
...
The demonstration focused on showing how users could follow selected workflow stages, inspect intermediate outputs, and review results through the prototype interface. In this way, the activity demonstrated through the Volvo CE case study the feasibility of representing selected parts of the workflow outside the SAP environment in a more transparent and inspectable form.
...
The final DSR activity was to evaluate the artefact. This activity aimed to determine whether the prototype fulfilled its intended purpose and satisfied the requirements defined earlier in the research process \cite{johannesson2021,hevner2004design}. Since the artefact was developed as a research prototype, the evaluation was formative and qualitative rather than a full production-level assessment. In this thesis, the artefact was evaluated through the Volvo CE case study.

In this activity, data were collected through prototype use, structured output inspection, and stakeholder feedback. The prototype was evaluated in a test setting using Volvo CE's 2024 market data and matrix files used for the yearly TMC calculation. The evaluation focused on the implemented workflow scope in the Volvo CE case, from source data upload to preparation, market view, adjustment, machine line split, and resplit.
