...
DSR was more suitable than a stand-alone case study because the thesis was not only about describing how the existing process works, but also about designing and evaluating an alternative representation of selected stages \cite{yin2018}. It was also more suitable than action research, since the project did not aim to intervene directly in Volvo CE's live production process or run an organizational change initiative \cite{susman1978}. More measurement-oriented approaches, such as surveys or controlled experiments, were likewise less suitable because the work depended on a small group of knowledgeable stakeholders and on detailed interpretation of workflow logic rather than on broad population-level comparison.

At the same time, the research clearly used the Volvo CE case as its empirical grounding. The broader design problem concerns transparency, traceability, and usability in ERP-based enterprise workflows, but that problem was studied through one concrete setting: the TMC process at Volvo CE. Within the overall DSR design, the Volvo CE setting functioned as an exploratory single-case setting with descriptive and evaluative elements. It was used to understand the workflow problem in context, guide artefact design, and assess the implemented prototype in a real organizational environment rather than to test causal explanations across multiple cases. In this thesis, DSR provides the overall research logic, while the case setting provides the practical context in which the problem, artefact, and evaluation can be understood.
...
In this activity, data were collected through prototype use, structured output inspection, and stakeholder feedback. The prototype was evaluated in a test setting using Volvo CE's 2024 market data and matrix files used for the yearly TMC calculation. The evaluation focused on the implemented workflow scope, from source data upload to preparation, market view, adjustment, machine line split, and resplit.
