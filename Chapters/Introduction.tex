...
\section{Background}
\pagestyle{plain}
Enterprise data-processing workflows are often more difficult to inspect than they are to run. In many large organizations, important analytical outputs depend on several source files, maintained mapping tables, and stepwise transformations that are executed inside enterprise systems such as SAP \cite{davenport1998,klaus2000,sap_erp_guide}. This is useful for operational control, but it can also make it hard for business users to see how a reported value was produced, which input was used, or where an inconsistency first appeared \cite{foidl2024pipeline}.

This kind of problem can also be found outside one specific company. Prior studies suggest that ERP- and SAP-based environments may become difficult to inspect when process logic is distributed across technical structures, transformation steps, and user-facing reporting layers \cite{weber2022sap,wong2016sap}. The construction equipment industry provides a relevant context because market analysis typically combines internal sales figures, external market data, and classification rules across countries, product categories, and machine segments \cite{cece2025,mordor2024construction}.
