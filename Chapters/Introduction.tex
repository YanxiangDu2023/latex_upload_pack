...
\section{Background}
\pagestyle{plain}
Enterprise data-processing workflows are often more difficult to inspect than they are to run. In many large organizations, important analytical outputs depend on several source files, maintained mapping tables, and stepwise transformations carried out in enterprise systems \cite{klaus2000}. In SAP-based environments, this supports operational control, but it can also make it hard for business users to see how a reported value was produced, which input was used, or where an inconsistency first appeared \cite{sap_erp_guide,foidl2024pipeline}.

This kind of problem can also be found outside one specific company. Prior studies suggest that ERP- and SAP-based environments may become difficult to inspect when process logic is distributed across technical structures, transformation steps, and user-facing reporting layers \cite{weber2022sap}. The construction equipment industry provides a relevant context because market analysis typically combines internal sales figures, external market data, and classification rules across countries, product categories, and machine segments \cite{cece2025}.

This thesis studies that broader issue through the case of Volvo Construction Equipment (Volvo CE). In the Volvo CE setting, market share estimation depends on a Total Market Calculation (TMC) process implemented in SAP. The process uses uploaded source data, maintained matrices, and staged calculations, but business users do not easily see how those elements connect to the intermediate and final outputs. In practice, this makes validation slower and makes discussions about unexpected results more dependent on technical specialists.

This broader problem is also reflected in prior work on SAP- and ERP-related environments, where business users may struggle to understand process logic, follow transformations across technical structures, and work efficiently with system outputs \cite{weber2022sap,wong2016sap}. In this thesis, \textit{transparency} refers to how clearly users can see the stages, rules, and intermediate outputs that shape a reported result. \textit{Traceability} refers to how far users can follow a result back to its source data, applied rules, and staged transformations. \textit{Usability} refers to how easily business users can navigate, inspect, validate, and discuss workflow results in practice.

The thesis therefore focuses on how selected parts of such a process can be shown in a form that is easier to inspect. A web-based prototype was designed as a research artefact and evaluated in the Volvo CE case study. The case is specific, but the underlying design problem concerns transparency, traceability, and usability in ERP-based enterprise data processing more generally.
...
This thesis asks how ERP-based enterprise data processing can be made easier for business users to inspect when such processing depends on heterogeneous inputs, maintained rules, and staged transformations. The Volvo CE TMC process is used as the empirical case study for that question.
