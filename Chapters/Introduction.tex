...
These problems are practically significant in the Volvo CE case, but they also reflect a broader design problem: enterprise data-processing workflows often prioritise execution correctness over inspectability for non-technical users. The motivation for this thesis therefore lies not only in improving one specific workflow, but in exploring how such workflows can be re-expressed in a form that better supports transparency, traceability, and validation. The Volvo CE setting provides a concrete case through which this more general problem can be investigated.
...
This thesis addresses a broader workflow-design problem: how complex enterprise data-processing workflows with heterogeneous inputs, embedded rules, and staged transformations can be represented in a way that is more transparent, traceable, and usable for business users. The Volvo CE TMC workflow is used as the empirical case because it provides a realistic example of this type of problem in an SAP-based enterprise environment.

The objective of the thesis is therefore to design and implement an interactive data processing and visualisation prototype that re-expresses selected workflow logic in a more inspectable form. In the case setting, this covers the preparation stage, including market-view derivation, as well as the adjustment and machine line split stages. More generally, the aim is to explore how workflow-oriented inspection, staged processing, and interactive result review can support enterprise data-processing tasks beyond one specific organisational context.


The main research question is:
How can an interactive data processing and visualisation prototype be designed and evaluated to enhance transparency, traceability, and usability in complex enterprise data-processing workflows?
This is addressed through three sub-questions:
...
This thesis focuses on the design and implementation of a web-based prototype for selected stages of the Volvo CE TMC workflow. The implemented scope covers source-data upload, matrix input handling, preparation, market-view derivation, adjustment, and selected split/resplit-related processing. The prototype provides interfaces for workflow navigation, layer-based visualisation, interactive table exploration, filtering, summary calculation, and CSV export.
